<nav class="navbar navbar-expand-lg main-navbar sticky">
        ...
        <ul class="navbar-nav navbar-right">
          ...
          @endphp
          <li class="dropdown dropdown-list-toggle"><a href="#" data-toggle="dropdown"
              class="nav-link notification-toggle nav-link-lg"><i data-feather="bell" class="bell"></i><span style="position: absolute; color: rgb(171, 168, 168); top: 0px; right: 5px; font-weight: 700;" >{{$ncount}}</span>
            </a>
                @php
                $user = Auth::user();
                @endphp
            <div class="dropdown-menu dropdown-list dropdown-menu-right pullDown">
              <div class="dropdown-header">
                Notifications
                <div class="float-right">
                  <a href="{{$user->unreadNotifications->markAsRead()}}">Mark All As Read</a>
                </div>
                
              </div>
              <div class="dropdown-list-content dropdown-list-icons">
                @foreach($user->notifications as $notification)
                <a href="{{$notification->markAsRead()}}" class="dropdown-item dropdown-item-unread"> <span
                    class="dropdown-item-icon bg-primary text-white"> <i class="fas fa-cart-plus"></i>
                  </span> <span class="dropdown-item-desc"> {{$notification->data['message']}} <span class="time">{{Carbon\Carbon::parse($notification->created_at)->diffForHumans()}}
                      </span>
                  </span>
                </a> 
                @endforeach
